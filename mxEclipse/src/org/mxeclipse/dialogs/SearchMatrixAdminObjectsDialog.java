...
public class SearchMatrixAdminObjectsDialog extends Dialog {
				...
				if (executed) {
					...
					if (tkzr.hasMoreTokens()) {
						do {
							...
							if ((nextToken.startsWith("Warning")) || (nextToken.startsWith("Error"))) {
								continue;
							}
							...
							if (pipePos != -1) {
								if (typeName.indexOf("Form") > -1) {
									...
									if ((typeName.equalsIgnoreCase("WebForm")) && ("TRUE".equalsIgnoreCase(webForm))) {
										retList.add(MxTreeBusiness.createBusiness(typeName, name));
									} else {
										if ((!typeName.equalsIgnoreCase("Form")) || (!"FALSE".equalsIgnoreCase(webForm))) {
											continue;
										}
										retList.add(MxTreeBusiness.createBusiness(typeName, name));
									}
					...
					} else if ((result != null) && (result.trim().length() > 0) && ((!typeName.equals("Association")) || (result.matches(namePattern.replace("*", ".*"))))) {
	...
	public static List<MxTreeBusiness> findAdminObjects(String type, String name) throws Exception {
